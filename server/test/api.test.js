...
try {
  ...
  const pune = dualModeStore.getCentre('PC-PUNE-01');
  ...
  const token = dualModeStore.getToken('A-127');
  ...
  const eta = QueueIntelligenceService.calculateFarmerETA('A-127', 'PC-PUNE-01');
  ...
  const proposal = SlotOptimizerService.getOptimizationProposal('PC-PUNE-01');
